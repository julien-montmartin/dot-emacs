...
On souhaite profiter de la recherche approximative, dans le cas d'un préfix mal orthographié ou écrit en abrégé. Cela fonctionne au minimum avec le contenu du buffer courant, mais manifestement pas avec les autres sources. Nécessite le paquet ~fuzzy~.

#+BEGIN_SRC emacs-lisp :tangle yes
(setq ac-use-fuzzy t)
#+END_SRC

Intègre Auto Complete à Helm, et permet d'avoir avec {{{CTRL-(:)}}} dans un buffer helm, le contenu du menu qu'on aurait eu avec {{{META-(/)}}}.
